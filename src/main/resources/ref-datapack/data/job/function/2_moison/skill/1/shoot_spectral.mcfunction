...
#清除之前的光灵箭
execute at @s at @e[type=spectral_arrow,distance=..600] if score @e[limit=1,sort=nearest] id = @s id run tag @e[sort=nearest,limit=1] add job_2_1_before
kill @e[tag=job_2_1_before]


execute as @s at @s anchored eyes positioned ^ ^ ^-1 run summon spectral_arrow ~ ~ ~ {Tags:["job_2_1_new","job_2_1"],HasBeenShot:1b,LeftOwner:1b,shake:0b,crit:0b,pickup:0b,damage:1.2,SoundEvent:"minecraft:entity.player.attack.crit"}
...
scoreboard players operation @e[tag=job_2_1_new] id = @s id
